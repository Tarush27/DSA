...
public class PrefixToPostfix {
    ...
    /*
    this function converts the given prefix expression to postfix taking exp as a param of type string which would return
    string
     */
    static String PreToPost(String exp) {
        ...
        for (int i = exp.length() - 1; i >= 0; i--) { // for loop will traverse from length of string to 0th index
            ...
            if (isOp(c)) { // this condition checks if the character 'c' is operator or not.
                ...
                String op2 = st.pop();
                String str = "(" + op1 + op2 + c + ")"; // concatenation of the operands , operator(at last) and the parans into a string.
                st.push(str); // pushing the resultant string in the stack.
            } else {
                st.push(c + ""); // pushing the operands in the stack.
            }
        }
        return st.peek(); // returning the last removed element from the stack.
    }
